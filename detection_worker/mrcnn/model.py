...
class MaskRCNN():
    ...
    def detect(self, images, verbose=0):
        ...
        detections, _, _, mrcnn_mask, _, _, _ = \
             self.keras_model.predict([molded_images, image_metas, anchors], verbose=0) #Changed predict to predict_on_batch to test memory leak.
        # self.keras_model.predict_on_batch([molded_images, image_metas, anchors], verbose=0)
        # Process detections
        results = []
...
class MeanAveragePrecisionCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if epoch > 0 and epoch % self.calculate_at_every_X_epoch == 0:
            ...
            mAPs,mPrecision,mRecall = self._calculate_mean_average_precision()
            mAP = np.mean(mAPs)

            if logs is not None:
                logs["val_mean_average_precision"] = mAP
                logs["val_mean_precision"] = mPrecision
                logs["val_mean_recall"] = mRecall
                

            self._verbose_print("mAP at epoch {0} is: {1}".format(epoch, mAP))
            self._verbose_print("precision at epoch {0} is: {1}".format(epoch, mPrecision))
            self._verbose_print("recall at epoch {0} is: {1}".format(epoch, mRecall))
    ...
    def _calculate_mean_average_precision(self):
        ...
        for image_id in self.dataset_image_ids[:self.dataset_limit]:
            ...
            AP, precisions, recalls, _ = utils.compute_ap(gt_bbox, gt_class_id, gt_mask, r["rois"],
                                           r["class_ids"], r["scores"], r['masks'])
            #print(mAPs)
            mAPs.append(AP)
            mPrecisions.append(np.mean(precisions))
            mRecalls.append(np.mean(recalls))
        
        #Clean the memory 
        keras.backend.clear_session()
        gc.collect()

        return np.array(mAPs), np.mean(mPrecisions), np.mean(mRecalls)