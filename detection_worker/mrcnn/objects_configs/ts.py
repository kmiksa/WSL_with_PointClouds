...
class TSDataset(utils.Dataset):

    def load_ts(self, dataset_dir, subset):
        """Load a subset of the ts dataset.
            ...
            if a.endswith('.json'):
                with open(dataset_dir + '/' + a, 'r') as f:
                    ...
                    for label in labels['Label']['traffic-sign']:
                        all_points_y = []
                        all_points_x = []
                        for coordinate in label['geometry']:
                            all_points_y.append(coordinate['y'])
                            all_points_x.append(coordinate['x'])
                        polygon = {}
                    ...
                    image_path = os.path.join(dataset_dir, labels['External ID'])
                    image = skimage.io.imread(image_path)
                    height, width = image.shape[:2]
                    self.add_image(
                        "ts",
                        image_id=labels['External ID'],
                        path=image_path,
...
def train(model):
    ...
    dataset_train.load_ts(args.dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = TSDataset()
    dataset_val.load_ts(args.dataset, "val")
    dataset_val.prepare()
