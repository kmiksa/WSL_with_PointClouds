...
class GarbageConfig(config.Config):
    ...
    NUM_CLASSES = 1 + 4  # Background + garbage
...
class GarbageDataset(utils.Dataset):
    ...
    def load_garbage(self, dataset_dir, subset):
        ...
        self.add_class("garbage", 1, "bag")
        self.add_class("garbage", 2, "cardboard")
        self.add_class("garbage", 3, "can")
        self.add_class("garbage", 4, "other")
                ...
                with open(dataset_dir + '/' + a, 'r') as f:
                    ...
                    for annotation in labels_dict['annotations']:
                        ...
                        for path in annotation['polygon']['path']:
                            all_points_y.append(path['y'])
                            all_points_x.append(path['x'])
                        polygon = {}
                        polygon['all_points_y'] = all_points_y
                        polygon['all_points_x'] = all_points_x
                        polygon['name'] = 'polygon'
                        polygon['class'] = annotation['name']
                        polygons.append(polygon)
                    # load_mask() needs the image size to convert polygons to masks.
                    # Unfortunately, VIA doesn't include it in JSON, so we must read
                    # the image. This is only managable since the dataset is tiny.
                    image_path = os.path.join(dataset_dir, labels_dict['image']['original_filename'])
                    image = skimage.io.imread(image_path)
                    height, width = image.shape[:2]
                    self.add_image(
                        "garbage",
                        image_id=labels_dict['image']['original_filename'],
                        path=image_path,
